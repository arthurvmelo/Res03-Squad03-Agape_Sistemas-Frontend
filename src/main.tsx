...
import { getQuilometragem } from "@/app/services/painel-service"
...
export default function Dashboard() {
  ...
  const [kmChartData, setKmChartData] = useState<any>({ labels: [], datasets: [] })

  const loadKmData = async () => {
    try {
      const response = await getQuilometragem(filters.vehicle)
      setKmChartData({
        labels: response.map((item: any) => item.mes),
        datasets: [
          {
            label: "Frota Empresa",
            data: response.map((item: any) => item.kmEmpresa),
            backgroundColor: "#1e293b",
          },
          {
            label: "Frota Terceirizada",
            data: response.map((item: any) => item.kmTerceirizados),
    ...
    <div className="min-h-screen bg-white">
      <div className="mx-auto flex max-w-7xl flex-col space-y-6 p-6">
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          >
            <div className="h-[260px] w-full">
              <Bar data={kmChartData} options={stackedBarOptions} />
            </div>
          </DashboardShellCard>
